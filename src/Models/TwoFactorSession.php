...
namespace CherryAnt\FilamentTwoFactor\Models;

use Filament\Facades\Filament;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\Casts\Attribute;
use CherryAnt\FilamentTwoFactor\Events\LoginSuccess;
...
class TwoFactorSession extends Model
{
    ...
    {
        session(['twofactor_session_id' => md5($this->id)]);
        // $this->update([
        //     'expires_at' => now()->addSeconds($lifetime ?? filament('filament-two-factor')->getTwoFactorSessionLifetime())
        // ]);
        // PLUS
        // $this->update([
    ...
    {
        return Attribute::make(
            get: fn () => session()->has('twofactor_session_id') && session('twofactor_session_id') == md5($this->id)
        );
        // return Attribute::make(
